...
of your server. Then you can access webapp at an URL like:
...
In examples below, '{host}', '{device}', '{variable}' represent host,
...
will query the device list registered in the `upsd` data server of the
specified host.
...
It will return a list of succinct descriptors of devices with `driver` and
