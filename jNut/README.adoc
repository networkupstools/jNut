...
level so most of environment can use it. Settings in `pom.xml` are however
higher (via properties you can change on ancient systems) so it is easy to
build by default on modern toolkits.
